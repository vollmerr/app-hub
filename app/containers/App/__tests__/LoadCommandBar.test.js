...
describe('<LoadCommandBar />', () => {
  ...
  describe('setCommandBar', () => {
    it('should set the command bar with items mapped out as disabled', () => {
      wrapper = shallow(<LoadCommandBar {...props} disabled />);
      jest.resetAllMocks();
      wrapper.instance().setCommandBar();
      const commandBar = {
    ...
    it('should set the command bar with unmodified commands if not disabled', () => {
      wrapper.instance().setCommandBar();
      expect(props.setCommandBar).toHaveBeenCalledWith(props.commandBar);
    });
  });
});
